...
export default function KotiJaRemontointiPage() {
  ...
  useEffect(() => {
    const haeIlmoitukset = async () => {
      ...
      let query = supabase
        .from('ilmoitukset')
        .select('*')
        .eq('kategoria', 'KotiJaRemontointi')
  ...
  useEffect(() => {
    const haePremiumit = async () => {
      ...
      const { data: ehdokkaat } = await supabase
        ...
        .eq('premium_kategoria', 'KotiJaRemontointi')
  ...
  return (
    <main className="max-w-screen-xl mx-auto p-6">
      <h1 className="text-2xl font-bold mb-6">Koti & Remontointi</h1>
            ...
            {premiumit.map((ilmo) => (
              <div key={ilmo.id} className="bg-[#f2f8f3] border border-[#3f704d] rounded-lg shadow-sm overflow-hidden">
                ...
                <div className="p-4">
                  ...
                  {!ilmo.id.toString().startsWith('tyhja') && (
                    <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {ilmoitukset.map((ilmo) => (
            <div key={ilmo.id} className="bg-white border rounded-lg shadow-sm overflow-hidden">
              <div className="h-32 w-full bg-gray-100 flex items-center justify-center">
                {ilmo.kuva_url ? (
                  <img src={ilmo.kuva_url} alt={ilmo.otsikko} className="h-full w-full object-cover" />
                ) : (
